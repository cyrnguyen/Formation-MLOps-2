...
def predict_endpoint():
    ...
    X = get_default_input(ws1_avg)

    features = prepare_features(data=X, training_mode=False)
    predictions = predict(features, MODEL_PATH)

    return jsonify({"result": extract_response(predictions)})



def extract_response(predictions):
    return predictions['predictions'][0]



def get_default_input(ws1_avg):
    return pd.DataFrame(
